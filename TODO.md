...
## 마무리 (2026-07-12)
- [x] 한국어 일반 질의가 조사 붙은 토큰을 못 찾는 문제 수정 — unicode61은 공백 단위 토큰화라 `예산안` 검색이 `예산안을`에 매칭되지 않았음. FTS5 문법이 없는 일반 질의는 단어별 접두사 매칭(`"예산안"*`)으로 자동 재작성, 연산자/따옴표/와일드카드가 있는 질의는 기존대로 그대로 통과 (`sbsearch.search`, `tests/test_search.py`)
- [x] 사용자용 README.md 작성
