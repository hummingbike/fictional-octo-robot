...
- [x] 대용량 log 파일 대응: 진짜 스트리밍 파싱 대신 PLAN.md 리스크 표의 "1차" 대응(크기 제외 옵션)을 구현 — `max_file_size_bytes`가 `indexer`/`watcher`/`semantic`의 색인 경로 전체에 일관되게 적용됨(`iter_matching_files`가 단일 진입점).
